...
export default function Ventas() {
  ...
  const productosFiltrados = productos.filter((p) => {
    const matchSearch =
      p.producto?.toLowerCase().includes(searchText.toLowerCase());

    const matchCategory =
      categoryFilter === "" ||
      p.categoria?.idCategoria == categoryFilter;
  ...
  return (
    <div className="flex flex-col md:flex-row w-full h-screen bg-gray-50 p-4 gap-4">
      ...
      <div className="w-full md:w-2/3 bg-white p-6 rounded-2xl shadow-md overflow-y-auto">
        <h1 className="text-2xl font-bold text-indigo-700 mb-4">Gestión de Ventas</h1>

        {/* Datos del cliente */}
        <div className="flex gap-2 mb-4">
          <input type="text" placeholder="Nombre del cliente (opcional)" value={cliente.nombre}
            onChange={(e) => setCliente({ ...cliente, nombre: e.target.value })}
            csNlasame="w-1/2 border border-gray-300 rounded-lg p-2 text-sm"/>

          <input type="number" placeholder="DNI (opcional)" value={cliente.documento}
            ...
            productosFiltrados.map((p) => (
              <div key={p.idProducto}
                ...
                <div className="w-24 h-24 bg-gray-200 rounded-lg flex items-center justify-center text-gray-500 text-sm mb-2">
                  {p.imagen ? (
                    <img src={`http://localhost:8500${p.imagen}`} alt={p.producto} className="w-full h-full object-cover rounded-lg" />
                  ) : (
                    "Sin Imagen"
                  )}
                </div>
                ...
                <p className="text-gray-600 text-sm">CATEGORÍA: {p.categoria?.categoria || "Sin categoría"}</p>
                <p className="font-bold text-purple-700 mb-2">S/ {p.precioVenta}</p>
                <button onClick={() => agregarAlCarrito(p)} className="bg-indigo-600 text-white rounded-lg px-3 py-1 text-sm hover:bg-indigo-700 transition">Agregar</button>
              </div>
            ))
          )}
        </div>
      ...
      <div className="w-full md:w-1/3 bg-white p-6 rounded-2xl shadow-md flex flex-col justify-between">
        <div>
          <h2 className="text-xl font-bold text-indigo-700 mb-4 flex items-center gap-2"> <ShoppingCart /> Carrito </h2>
          ...
          {carrito.length === 0 ? (
            ...
            <ul className="divide-y">
              {carrito.map((item) => (
                <li key={item.idProducto} className="py-2 flex justify-between items-center text-sm">
                  <div className="flex items-center gap-2">
                    <span className="font-medium">{item.producto}</span>

                    {/* Botones para modificar cantidad */}
                    <button onClick={() => setCarrito(carrito.map(p => p.idProducto === item.idProducto
                            ? { ...p, cantidad: p.cantidad - 1 > 0 ? p.cantidad - 1 : 1 } : p )) }
                  ...
                  <div className="flex items-center gap-2">
                    ...
                    <button
                      onClick={() => setCarrito(carrito.filter(p => p.idProducto !== item.idProducto))}
        ...
        <div className="mt-6">
          ...
          > <ShoppingCart /> Finalizar Venta </button>
        </div>
      </div>
    </div>
  );
